...
public:
  /// Creates a new FBO and associated 2D textures.
  /// Each color attachment has the default GLTexture2D internal format (GL_RGBA8).
  /// The depth attachment, if created, has an internal format of GL_DEPTH_COMPONENT24.
  /// @arg size The size of the underlying textures in pixels.
  ...
  /// Create a framebuffer that takes ownership of existing textures.
  /// Arguments should be transfered using std::move().
  ...
  /// Create a framebuffer, initializing the texture attachments given the provided formats.
  ...
  ///   texture.
  /// @arg depthAttachmentFormat A pair of internalFormat and format parameters for the depth
  ///   texture. This may be null.
  Framebuffer(const Eigen::Vector2i& size,
  ...
  /// Moves tracked GPU resources into this instance. The other instance is left empty.
  Framebuffer(Framebuffer&& other);

  /// Moves tracked GPU resources into this instance. Any previously owned resources are released
  /// and the other instance is left empty.
  Framebuffer& operator=(Framebuffer&& other);

  /// Deletes this framebuffer and its associated textures.
  ...
  /// Return the size of this FBO in pixels.
...
inline
const Eigen::Vector2i& Framebuffer::size() const {
  return mSize;
}
