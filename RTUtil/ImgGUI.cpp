...
namespace RTUtil
{
  ...
  ImgGUI::ImgGUI(int width, int height) : windowWidth(width),
                                          windowHeight(height),
                                          nanogui::Screen(Eigen::Vector2i(width, height), "NanoGUI Demo", false)
  {
    ...
    try
    {
      srgbShader.reset(new GLWrap::Program("srgb", {{GL_VERTEX_SHADER, fsqVertSrcPath},
  ...
  bool ImgGUI::mouseMotionEvent(const Eigen::Vector2i &p, const Eigen::Vector2i &rel, int button, int modifiers)
  {
    return false;
  }
